...
/*
    ___setTimeout (Function):
        This function is similar to the native 'setTimeout' function 
        such that our tracing history objects don't include reports 
        of the tracing loop '___watchLoop' in '___startTrace'.
*/
		...
		for(var ___i in traceObject) {
			if(!___props.hasOwnProperty(___i)) {
				...
				if(___addToWatchList(___i.toString(), traceObject)) {
					...
					___propertyIntercept(___i, null, traceObject[___i]);
				}
			} else {
...
// setTimeout(___showTrace, 10000); //demo...
